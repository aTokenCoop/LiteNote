...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 250000 blocks, which will occur approximately every year
    nSubsidy >>= (nHeight / 250000); // Clonecoin: 250k blocks in ~1 year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.5 * 24 * 60 * 60; // Litecoin: 1.5 days
static const int64 nTargetSpacing = 2 * 60; // Litecoin: 2.5 minutes
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 02/Mar/2018 China Presses Its Internet Censorship Efforts Across the Globe";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
